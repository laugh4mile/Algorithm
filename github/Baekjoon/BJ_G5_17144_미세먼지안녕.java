...
public class BJ_G5_17144_미세먼지안녕 {
	...
	static int R,C,T,map[][],calmap[][],cCnt, Spintemp[][];
...
//		System.out.println("확산후map");
		...
		System.out.println("--------------------------------");
			for(int r=0; r<R; r++) {
				for(int c=0; c<C; c++) {
					Spintemp[r][c] = calmap[r][c];
				}	
			}
			calmap[cleaner[0].x][cleaner[0].y] = 0;
			calmap[cleaner[1].x][cleaner[1].y] = 0;
			spin();
			for(int []x: calmap) {
				System.out.println(Arrays.toString(x));
			}
	...
	private static void spin() { 
		for(int r=0; r<=cleaner[0].x; r++) { 
			for(int c=0; c<C; c++) { 
				if(r>0 && c==0) { // 왼쪽 
					Spintemp[r][c] = calmap[r-1][c]; 
				}else if(c<C-1 && r==0) { // 위 // 잘됨
					Spintemp[r][c] = calmap[r][c+1];
	...
	private static void spread(int r, int c) {
		...
		for(int d=0; d<4; d++) {
			...
			if(isIn(nr, nc) && map[nr][nc] != -1) {
				visited[nr][nc] = true;
				calmap[nr][nc] += map[r][c]/5;
				cntSpread++;
			}
		}
		calmap[r][c] += map[r][c] - (map[r][c]/5)*cntSpread;
	}
