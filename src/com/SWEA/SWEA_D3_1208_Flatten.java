 ...
 * @author itsme
 * @date 2020. 7. 28
 * @see https://swexpertacademy.com/main/talk/solvingClub/problemView.do?contestProbId=AV139KOaABgCFAYh&solveclubId=AXOJsQN67-MDFAXS&problemBoxTitle=0728&problemBoxCnt=1&probBoxId=AXOSG8Ga1tEDFAXS
 * @mem
 * @time
 * @caution #단순 배열
 * [고려사항]
 * 가장 높은 곳에 있는 박스를 가장 낮은 곳으로 이동시키기 --> 반복문 내에서 정렬 처리 후 이동
 * [입력사항]
 * [출력사항]
 */
...
//		input = new BufferedReader(new StringReader(src));  // 제출 할 때는 당연히 주석처리.
		
		for(int t=1; t<=10; t++) {
			int maxDumpCnt = Integer.parseInt(input.readLine()); // 문자열 --> Integer
...
//			System.out.println(Arrays.toString(box));
			...
			// 정답에 대한 초기화를 해주는게 안전하다
			answer = 0;
			
			for(int d=0; d<maxDumpCnt; d++) {
				//정렬
				Arrays.sort(box);
				if(box[99]-box[0]>1) {
					box[99]--;
